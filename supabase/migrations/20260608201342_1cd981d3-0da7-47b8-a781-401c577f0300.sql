...
BEGIN
  ...
  SELECT COUNT(DISTINCT qi.id) INTO v_queue_with_routing
    FROM public.queue_items qi
    JOIN public.work_order_routing wor ON wor.queue_item_id = qi.id
   WHERE qi.organization_id = p_org_id;

  SELECT COALESCE(logo_light_url,'') <> '' OR COALESCE(logo_dark_url,'') <> '' INTO v_branding_logo
    FROM public.organization_branding
    WHERE organization_id = p_org_id LIMIT 1;
  v_branding_logo := COALESCE(v_branding_logo, false);

  IF v_dept    = 0 THEN v_blockers := array_append(v_blockers, 'No departments configured'); END IF;
  IF v_station = 0 THEN v_blockers := array_append(v_blockers, 'No active stations'); END IF;
  IF v_equip   = 0 THEN v_blockers := array_append(v_blockers, 'No equipment registered'); END IF;
  IF v_admin   = 0 THEN v_blockers := array_append(v_blockers, 'No org admin assigned'); END IF;
  IF v_operator = 0 THEN v_blockers := array_append(v_blockers, 'No operator/supervisor members'); END IF;
  IF v_routing = 0 THEN v_blockers := array_append(v_blockers, 'No routing templates with steps'); END IF;
  IF v_branding = 0 THEN v_blockers := array_append(v_blockers, 'Organization branding missing'); END IF;
  IF v_sub = 0 THEN v_blockers := array_append(v_blockers, 'No active subscription tier'); END IF;
  IF v_itar AND v_persistence <> 'read_through' THEN
    v_blockers := array_append(v_blockers, 'ITAR org must use read-through ERP persistence');
  END IF;
  IF v_station_with_equip = 0 THEN v_blockers := array_append(v_blockers, 'No station has equipment assigned'); END IF;
  IF v_operator_signed_in = 0 THEN v_blockers := array_append(v_blockers, 'No member has signed in yet'); END IF;
  IF v_queue_with_routing = 0 THEN v_blockers := array_append(v_blockers, 'No queue items have a routing applied'); END IF;
  IF NOT v_branding_logo THEN v_blockers := array_append(v_blockers, 'Branding logo is missing'); END IF;

  RETURN jsonb_build_object(
    'ready', array_length(v_blockers,1) IS NULL,
    'blockers', to_jsonb(v_blockers),
    'counts', jsonb_build_object(
      'departments', v_dept, 'stations', v_station, 'equipment', v_equip,
      'admins', v_admin, 'operators', v_operator,
      'routing_templates', v_routing, 'branding', v_branding,
      'subscriptions', v_sub, 'erp_connections', v_erp_conn,
