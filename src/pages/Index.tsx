...
const Index = () => {
  ...
  // F-5: Talent-only users (signed-in, confirmed no organization) → redirect to talent dashboard.
  // Wait for orgLoading to finish so a transient null doesn't bounce the user erroneously.
  useEffect(() => {
    if (isReady && user && !orgLoading && !organization && !onboardingLoading) {
      navigate("/talent/dashboard", { replace: true });
    }
  }, [isReady, user, orgLoading, organization, onboardingLoading, navigate]);
