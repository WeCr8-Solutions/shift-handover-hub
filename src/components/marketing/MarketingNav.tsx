...
type MenuKey = "products" | "industries" | "learn" | null;
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border bg-background px-4 py-3 space-y-1 max-h-[80vh] overflow-y-auto">
          ...
          <MobileSection title="Products">
            {productItems.map((p) => (
              <button key={p.href} onClick={() => go(p.href)} className="block w-full text-left px-3 py-2 text-sm text-foreground hover:bg-accent rounded-md">
                {p.label}
              </button>
            ))}
          </MobileSection>
