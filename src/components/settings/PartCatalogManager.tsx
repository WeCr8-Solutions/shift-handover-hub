...
export function PartCatalogManager() {
  ...
  const fetchEntries = useCallback(async () => {
    ...
    if (search.trim()) {
      query = query.or(`part_number.ilike.%${search}%,description.ilike.%${search}%,material_type.ilike.%${search}%`);
    }
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      if (error) { toast.error(error.message); } else { toast.success("Part updated"); }
    } else {
      const { error } = await supabase.from("part_catalog").insert(payload);
      if (error) { toast.error(error.message); } else { toast.success("Part created"); }
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) { toast.error(error.message); } else { toast.success("Part deleted"); fetchEntries(); }
  };
      ...
      <CardContent className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs mt-1">Add parts to auto-fill specs on work orders</p>
          </div>
        ) : (
              ...
              <TableBody>
                {entries.map((entry) => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell>
                      {entry.material_type ? (
                        <Badge variant="secondary" className="text-xs">{entry.material_type}</Badge>
                      ) : "—"}
                    </TableCell>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>Part Number <span className="text-destructive">*</span></Label>
                <Input value={formData.part_number} onChange={(e) => setFormData({ ...formData, part_number: e.target.value })} placeholder="e.g. 12345-A" />
              </div>
              <div className="space-y-2">
                <Label>Material Type</Label>
                <Select value={formData.material_type || "none"} onValueChange={(v) => setFormData({ ...formData, material_type: v === "none" ? "" : v })}>
            ...
            <div className="space-y-2">
              ...
              <Textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Optional part description" rows={2} />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>Part Shape</Label>
                <Select value={formData.part_shape || "none"} onValueChange={(v) => setFormData({ ...formData, part_shape: v === "none" ? "" : v })}>
                  <SelectTrigger><SelectValue placeholder="Select..." /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">Not specified</SelectItem>
                    {PART_SHAPES.map((s) => (<SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label>Weight (lbs)</Label>
                <Input type="number" value={formData.part_weight_lbs} onChange={(e) => setFormData({ ...formData, part_weight_lbs: e.target.value })} placeholder="lbs" min="0" step="0.01" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2">
                ...
                <Input type="number" value={formData.part_length_inches} onChange={(e) => setFormData({ ...formData, part_length_inches: e.target.value })} placeholder="L" min="0" step="0.001" />
              </div>
              <div className="space-y-2">
                <Label>Width (in)</Label>
                <Input type="number" value={formData.part_width_inches} onChange={(e) => setFormData({ ...formData, part_width_inches: e.target.value })} placeholder="W" min="0" step="0.001" />
              </div>
              <div className="space-y-2">
                <Label>Height (in)</Label>
                <Input type="number" value={formData.part_height_inches} onChange={(e) => setFormData({ ...formData, part_height_inches: e.target.value })} placeholder="H" min="0" step="0.001" />
              </div>
            </div>
