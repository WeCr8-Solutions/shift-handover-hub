...
const CATEGORIES = ["Operations", "Shop Floor", "CNC", "Production", "Quality", "Planning", "General"];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create and manage blog posts. MDX seed posts from the codebase are also shown on the public blog.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            No database posts yet. Click "New Post" to create one, or posts from MDX files will still appear on the blog.
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-4 flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge variant={post.is_published ? "default" : "secondary"} className="shrink-0 text-xs">
                      {post.is_published ? "Published" : "Draft"}
                    </Badge>
                    <Badge variant="outline" className="shrink-0 text-xs">{post.category}</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground truncate">/blog/{post.slug} · {post.author} · {post.published_date}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Button variant="ghost" size="icon" className="text-destructive" onClick={() => openDelete(post)}>
