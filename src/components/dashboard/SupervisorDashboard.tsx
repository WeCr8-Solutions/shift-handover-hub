...
export function SupervisorDashboard({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Alerts Area: WO + Station side-by-side (desktop) / stacked (mobile) ── */}
      {(woAlerts.length > 0 || stationAlerts.length > 0 || attentionItems.length > 0) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Work Order Alerts */}
          {(woAlerts.length > 0) && (
            <Collapsible open={woAlertsOpen} onOpenChange={setWoAlertsOpen}>
              <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          {/* Station / Machine Alerts (smart + attention items merged) */}
          {(stationAlerts.length > 0 || attentionItems.length > 0) && (
            <Collapsible open={stationAlertsOpen} onOpenChange={setStationAlertsOpen}>
              <div className="bg-card border border-border rounded-lg overflow-hidden">
                <CollapsibleTrigger asChild>
                  <button className="w-full px-4 py-3 border-b border-border bg-secondary/30 flex items-center justify-between hover:bg-secondary/50 transition-colors">
                    <div className="flex items-center gap-2">
