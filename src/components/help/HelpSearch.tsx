...
export function HelpSearch({ className, large }: HelpSearchProps) {
        ...
        <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-popover border border-border rounded-lg shadow-lg max-h-80 overflow-y-auto">
          {results.slice(0, 8).map((article) => (
            <button
              ...
              onMouseDown={() => navigate(`/help/${article.category}/${article.slug}`)}
            >
              <p className="text-sm font-medium text-foreground">{article.title}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{article.categoryLabel} · {article.description}</p>
            </button>
          ))}
        </div>
      )}
