...
export const helpArticles: HelpArticle[] = [
  ...
  // ════════════════════════════════════════════════════════════════
  {
    ...
    sections: [
      ...
      { heading: "Accessing the Tools Hub", body: "Navigate to /tools from the main navigation or sidebar. The hub displays all available tools as a searchable, filterable card grid plus a dedicated Machinist's Reference Library entry point. Use the category tabs (Machining, Measurement, Conversion, Reference) to narrow the list, or type keywords into the search bar to find a specific tool. Click any calculator card to open it in a slide-out panel, or open the handbook to browse longer-form reference content." },
      { heading: "Machinist's Reference Library", body: "The handbook is JobLine.ai's digital machinery-reference layer. It centralizes live reference material for feeds and speeds, threads, fits and tolerances, GD&T, formulas, inspection practices, tooling, and safety standards. This is where JobLine will keep expanding worldwide manufacturing knowledge in original, source-cited form rather than scattered across separate PDFs or notes." },
      { heading: "Used Across JobLine", body: "The handbook is not a standalone page only. The same reference layer is used throughout the application to support operator decisions in context. OAP lesson sections can render handbook citations for the topic being checked off, and G-Code Academy question review can surface the matching handbook references after a learner answers. The handbook search in the main header also lets operators jump straight to reference material from anywhere in the app." },
